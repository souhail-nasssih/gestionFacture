...
<body>
        ...
        <tr>
            ...
            <td class="top-left">
                <div class="info-block">
                    <div><strong>Date du Bon de Livraison:</strong> {{ \Carbon\Carbon::parse($blFournisseur->date_bl)->format('d/m/Y') }}</div>
            ...
            <td class="top-right">
                <div class="destinataire info-block" style="text-align:left;">
                    ...
                    <div><span class="label">Société</span> <strong>{{ $blFournisseur->fournisseur->nom }}</strong></div>
                    ...
                    <div><span class="label">Tel</span> {{ $blFournisseur->fournisseur->telephone ?? '' }}</div>
                    <div><span class="label">ICE</span> {{ $blFournisseur->fournisseur->ice ?? '' }}</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="products">
        ...
        <tbody>
            @php $rows = 0; @endphp
            @foreach ($blFournisseur->details as $detail)
                <tr>
                    ...
                    <td class="text-right">
                        {{ number_format($detail->montant_bl ?? $detail->quantite * ($detail->prix_unitaire ?? 0), 2, ',', ' ') }}
                        DH</td>
                </tr>
                @php $rows++; @endphp
            @endforeach
    ...
    <!-- TOTAL -->
    @php
        $totalHT = $blFournisseur->details->sum(function($detail) {
            return $detail->montant_bl ?? ($detail->quantite * $detail->prix_unitaire);
        });
        $tvaRate = 20;
